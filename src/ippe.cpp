...
std::vector<cv::Mat> IPPE::solvePnP(const std::vector<cv::Point3f>& objPoints, const std::vector<cv::Point2f>& imgPoints,
                                     ...
                                     cv::InputArray cameraMatrix, cv::InputArray distCoeffs)
{
  ...
  IPPE::solvePoseOfCentredSquare(markerLength, imgPoints, cameraMatrix, distCoeffs, Rvec, Tvec, reprojErr1, Rvec2,
                                 Tvec2, reprojErr2);
  return { std::make_pair(getRTMatrix(Rvec, Tvec, CV_32F), reprojErr1),
           std::make_pair(getRTMatrix(Rvec2, Tvec2, CV_32F), reprojErr2)};
}

void IPPE::solvePoseOfCentredSquare(float squareLength, cv::InputArray imagePoints, cv::InputArray cameraMatrix,
                                    cv::InputArray distCoeffs, cv::OutputArray _rvec1, cv::OutputArray _tvec1, float& reprojErr1,
...
void IPPE::IPPComputeRotations(double j00, double j01, double j10, double j11, double p, double q, cv::OutputArray _R1,
                               cv::OutputArray _R2)
{
...
void IPPE::homographyFromSquarePoints(cv::InputArray _targetPts, double halfLength, cv::OutputArray H_)
{
